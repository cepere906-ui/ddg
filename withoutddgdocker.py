cat > /root/cdn_docker.py << 'ENDOFFILE'
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
Docker CDN - Simple Reverse Proxy in Container
===============================================
Автоматическая установка Docker и запуск Nginx в контейнере
"""
...
class DockerCDN:
    ...
    def install_docker(self):
        ...
        try:
            # Используем официальный скрипт Docker
    ...
    def create_nginx_config(self):
        ...
        self._print_step("Создание конфигурации Nginx...")
...
"""
        ...
        print(f"{Colors.GREEN}✓ Nginx конфиг создан: {self.nginx_conf}{Colors.NC}")
...
"""
    ...
    def start_docker_compose(self):
        ...
        self._print_step("Запуск Docker контейнера...")
        
        # Остановка старых контейнеров (Compose v2)
        ...
        if result.returncode == 0:
            ...
            # Проверка статуса
            status = self._run_command(f"docker ps --filter name={self.container_name} --format '{{{{.Status}}}}'", check=False)
    ...
    def show_status(self):
        ...
        print(f"\n{Colors.CYAN}▶ Логи (последние 10 строк):{Colors.NC}")
        self._run_command(f"docker logs --tail 10 {self.container_name}", check=False)
    
    def cleanup(self):
        """Остановка и очистка"""
        print(f"\n{Colors.MAGENTA}>>> CLEANUP <<<{Colors.NC}")
        
        # Остановка контейнера (Compose v2)
    ...
    def run(self):
        ...
        print(f"{Colors.CYAN}DOCKER CDN - Nginx Reverse Proxy в контейнере{Colors.NC}")
        print(f"{Colors.CYAN}{'='*70}{Colors.NC}")
        ...
        if self.start_docker_compose():
            ...
            print(f"{Colors.GREEN}║ {'CDN ЗАПУЩЕН В DOCKER':^68} ║{Colors.NC}")
            ...
            print(f"\n{Colors.MAGENTA}💡 Для добавления DDoS защиты:{Colors.NC}")
            print(f"  1. Редактируй: {self.nginx_conf}")
            print(f"  2. Перезапускай: docker compose restart")
            print(f"  3. Всё изолировано - сервер не пострадает!")
...
if __name__ == '__main__':
    cdn = DockerCDN(TARGET_DOMAIN, ORIGIN_IP, ORIGIN_PORT)
    cdn.run()
ENDOFFILE

chmod +x /root/cdn_docker.py
python3 /root/cdn_docker.py
